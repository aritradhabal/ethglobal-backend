...
export class CompleteGraphService {
  ...
  private propertyIndexes: Map<string, Map<PropertyValue, Set<string>>> =
    new Map();
  ...
  createRelationship(
    ...
    console.log(
      `✅ Created relationship: ${sourceId} -[${type}]-> ${targetId}`
    );
    return id;
  }
  ...
  updateRelationship(
    ...
    if (updates.properties)
      rel.properties = { ...rel.properties, ...updates.properties };
  ...
  public getOutgoingRelationships(sourceId: string, type: string) {
    const rels = Array.from(this.relationships.values()).filter(
      (r) => r.sourceId === sourceId
    );
    return type ? rels.filter((r) => r.type === type) : rels;
  }
  public getIncomingRelationships(targetId: string, type?: string) {
    const rels = Array.from(this.relationships.values()).filter(
  ...
  query(query: GraphQuery): QueryResult {
    ...
    try {
      ...
      if (query.where && query.where.length > 0) {
        candidateNodes = this.applyWhereClausesToNodes(
          candidateNodes,
          query.where
        );
        candidateRelationships = this.applyWhereClausesToRelationships(
      ...
      if (query.return && query.return.length > 0) {
        const returnResult = this.processReturnClauses(
          candidateNodes,
          candidateRelationships
        );
  ...
  findShortestPath(
    ...
    while (queue.length > 0) {
      ...
      if (current.nodeId === targetId) {
        ...
        const pathRels = current.relationships.map(
  ...
  calculatePageRank(
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      nodes.forEach((nodeId) => {
        let sum = 0;
        const incomingNeighbors = this.getNodeNeighbors(nodeId, {
          direction: "in",
        });
  ...
  getGraphStats(): GraphStats {
    ...
    const nodeTypes = [
      ...new Set(Array.from(this.nodes.values()).map((n) => n.type)),
    ];
    const relationshipTypes = [
      ...new Set(Array.from(this.relationships.values()).map((r) => r.type)),
    ];
    ...
    const density =
      maxPossibleEdges > 0 ? (relationshipCount * 2) / maxPossibleEdges : 0;
  ...
  async loadGraph(blobId: string): Promise<void> {
    try {
      ...
      const { nodes, relationships } = await this.walrusService.readGraph(
        blobId
      );
    ...
    this.relationships.forEach((rel) => {
      ...
      if (isValidDirection) {
        ...
        if (
          options?.nodeTypes &&
          neighborNode &&
          !options.nodeTypes.includes(neighborNode.type)
        ) {
          return;
        }
      ...
      if (pattern.nodePattern) {
        if (pattern.nodePattern.labels) {
          nodes = nodes.filter((node) =>
            pattern.nodePattern!.labels!.every((label) =>
        ...
        if (pattern.nodePattern.properties) {
          nodes = nodes.filter((node) =>
            Object.entries(pattern.nodePattern!.properties!).every(
      ...
      if (pattern.relationshipPattern) {
        if (pattern.relationshipPattern.type) {
          relationships = relationships.filter(
            (rel) => rel.type === pattern.relationshipPattern!.type
          );
        }
      }
    });
  ...
  private applyWhereClausesToNodes(
    nodes: GraphNode[],
