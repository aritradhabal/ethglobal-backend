...
export class WalrusService {
  ...
  async readGraph(
    ...
    try {
      ...
      const nodes: GraphNode[] = (graphData.nodes || []).map(
        (node: {
          id?: string;
          type?: string;
          properties?: Record<string, unknown>;
  ...
  private async storeBlob(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Walrus HTTP error");
      }
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        throw new Error("❌ Network/CORS error connecting to Walrus:");
      } else {
        throw new Error("Unexpected error with Walrus");
      }
    }
  }
  ...
  private storeToLocalStorage(
    ...
    const blobId = `local_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  private async convertToJsonLD(
    ...
    const jsonldRelationships: JsonLdRelationship[] = relationships.map(
      (rel) => ({
        "@type": "Relationship",
        "@id": rel.id,
   ...
   */
  private async convertFromJsonLD(
    ...
    try {
      ...
      const relationships: GraphRelationship[] = (
        Array.isArray(relationshipsData) ? relationshipsData : []
      ).map(
        (relData: {
          "@id"?: string;
          id?: string;
          ...
          const properties =
            typeof relData.properties === "object" &&
            relData.properties !== null
              ? relData.properties
              : {};
      ...
      console.log(
        `✅ Converted JSON-LD: ${nodes.length} nodes, ${relationships.length} relationships`
      );
      return { nodes, relationships };
    } catch (error) {
   ...
   */
  private fallbackParse(data: Record<string, unknown>): {
    nodes: GraphNode[];
    relationships: GraphRelationship[];
  } {
    const nodes: GraphNode[] = (
